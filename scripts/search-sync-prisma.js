 ...
 * Uses Spotify search + Prisma client (not raw pg).
 */
...
async function main() {
  console.log("SLC Search-Based Release Sync (Prisma)");
  console.log("======================================\n");
  ...
  for (const artist of ARTISTS) {
    ...
    for (const r of newReleases) {
      allNewReleases.push({ ...r, artistId: dbArtist.id, artistName: artist.name });
    }
  }
  
  // Insert new releases
  console.log(`\n\nInserting ${allNewReleases.length} new releases...`);
  ...
  // Print final counts
  const byArtist = await prisma.artist.findMany({
  ...
  for (const a of byArtist) {
    console.log(`  ${a.name}: ${a._count.releases}`);
    total += a._count.releases;
  }
