...
export async function initializeDatabase(): Promise<void> {
  ...
  `);
  // Create curated_spotify_channels table for tracking Spotify artists to sync
  await db.execute(`
    CREATE TABLE IF NOT EXISTS curated_spotify_channels (
      ...
      popularity INTEGER DEFAULT 0,
      followers INTEGER DEFAULT 0,
      category TEXT DEFAULT 'roster',
      priority INTEGER DEFAULT 0,
      description TEXT,
      auto_sync INTEGER DEFAULT 1,
      sync_new_releases INTEGER DEFAULT 1,
      sync_top_tracks INTEGER DEFAULT 1,
      is_active INTEGER DEFAULT 1,
      last_synced_at TEXT,
      created_at TEXT DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT DEFAULT CURRENT_TIMESTAMP
