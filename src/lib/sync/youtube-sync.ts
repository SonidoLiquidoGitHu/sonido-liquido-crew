...
import { syncJobsRepository, videosRepository } from "@/lib/repositories";
...
export async function syncYouTube(
  ...
  try {
    ...
    if (options.channelIds && options.channelIds.length > 0) {
      ...
      channelInfos = options.channelIds.map((id) => ({
        channelId: id,
        artistId: null,
      }));
    } else {
      ...
      try {
        ...
        for (const channel of channels) {
          ...
          if (channel.channelId.startsWith("UC")) {
            ...
            const channelInfo = YouTubeClient.extractChannelInfo(
              channel.channelUrl,
            );
            if (channelInfo) {
              if (channelInfo.type === "id") {
                ...
                try {
                  const resolved = await youtubeClient.getChannelByHandle(
                    channelInfo.value,
                  );
                  if (resolved) {
                    channelInfos.push({
                    ...
                    try {
                      await db
                        ...
                        .set({
                          channelId: resolved.id,
                          channelName:
                            resolved.snippet?.title || channel.channelName,
                          thumbnailUrl:
                            resolved.snippet?.thumbnails?.high?.url ||
                            resolved.snippet?.thumbnails?.default?.url ||
                            channel.thumbnailUrl,
                          subscriberCount: resolved.statistics?.subscriberCount
                            ? Number.parseInt(
                                resolved.statistics.subscriberCount,
                                10,
                              )
                      ...
                      console.warn(
                        `[YouTube Sync] Could not update channel ${channel.id} with resolved ID:`,
                        updateErr,
                      );
                    }
                  }
                  ...
                  result.errors.push(
                    `Failed to resolve handle @${channelInfo.value} for channel ${channel.channelName}`,
                  );
                }
              }
            }
        ...
        await syncJobsRepository.addLog(
          syncJob.id,
          "warning",
          `Could not read youtube_channels table: ${(dbErr as Error).message}`,
        );
      }
      ...
      try {
        ...
        const coveredArtistIds = new Set(
          channelInfos.map((c) => c.artistId).filter(Boolean),
        );
            ...
            if (channelInfo) {
              if (channelInfo.type === "id") {
                ...
                try {
                  ...
                  result.errors.push(
                    `Failed to resolve channel handle @${channelInfo.value} for ${artist.name}`,
                  );
                }
              }
            }
        ...
        await syncJobsRepository.addLog(
          syncJob.id,
          "warning",
          `Could not read artist profiles: ${(dbErr as Error).message}`,
        );
      }
    }
    ...
    if (channelInfos.length === 0) {
      result.errors.push(
        "No YouTube channels found. Add channels via the admin YouTube Channels page, or add YouTube URLs to artist profiles.",
      );
    }
    ...
    for (const { channelId, artistId } of channelInfos) {
      try {
        await syncJobsRepository.addLog(
          syncJob.id,
          "info",
          `Syncing videos from channel ${channelId}`,
        );
          ...
          try {
            ...
            if (existing) {
              ...
              await videosRepository.update(existing.id, {
                ...
                duration: YouTubeClient.parseDuration(
            ...
            result.videosFailed++;
            result.errors.push(
              `Failed to sync video ${video.snippet.title}: ${(error as Error).message}`,
            );
          }
        }
      } catch (error) {
        result.errors.push(
          `Failed to fetch videos from channel ${channelId}: ${(error as Error).message}`,
        );
        await syncJobsRepository.addLog(
          syncJob.id,
          "error",
