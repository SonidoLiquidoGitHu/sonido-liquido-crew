 ...
 */
export async function getDropboxUploadToken(): Promise<{
  token: string;
  error?: string;
}> {
  try {
    ...
    if (!data.success || !data.data?.token) {
      return {
        token: "",
        error: data.error || "No se pudo obtener el token de Dropbox",
      };
    }
...
async function uploadLargeFile(
  ...
  console.log(
    `[Dropbox Browser] Starting chunked upload: ${totalChunks} chunks for ${(file.size / 1024 / 1024).toFixed(1)}MB file`,
  );
  ...
  try {
    ...
    if (!startResponse.ok) {
      ...
      console.error(
        "[Dropbox Browser] Upload session start failed:",
        errorText,
      );
      if (startResponse.status === 401) {
        return {
          success: false,
          error:
            "Token de Dropbox expirado. Ve a Sincronización > Dropbox y reconecta.",
        };
      }
      return {
        success: false,
        error: `Error al iniciar subida: ${startResponse.status}`,
      };
    }
    ...
    if (file.size > CHUNK_SIZE) {
      ...
      while (offset < file.size) {
        ...
        console.log(
          `[Dropbox Browser] Uploading chunk ${chunkNumber}/${totalChunks} (offset: ${offset})`,
        );
        ...
        if (isLastChunk) {
          ...
          if (!finishResponse.ok) {
            ...
            console.error(
              "[Dropbox Browser] Upload session finish failed:",
              errorText,
            );
            return {
              success: false,
              error: `Error al finalizar subida: ${finishResponse.status}`,
            };
          }

          const finishData = await finishResponse.json();
          console.log(
            "[Dropbox Browser] Chunked upload completed:",
            finishData.path_display,
          );
        } else {
          // Append chunk
          ...
          if (!appendResponse.ok) {
            ...
            console.error(
              "[Dropbox Browser] Upload session append failed:",
              errorText,
            );
            return {
              success: false,
              error: `Error al subir parte ${chunkNumber}: ${appendResponse.status}`,
            };
          }
        }
      ...
      if (!finishResponse.ok) {
        const errorText = await finishResponse.text();
        console.error(
          "[Dropbox Browser] Upload session finish (single chunk) failed:",
          errorText,
        );
        return {
          success: false,
...
export async function uploadToDropboxDirect(
  ...
  try {
    ...
    if (tokenError || !token) {
      return {
        success: false,
        error: tokenError || "No se pudo obtener el token",
      };
    }
    ...
    if (useChunkedUpload) {
      console.log(
        `[Dropbox Browser] Using chunked upload for ${(file.size / 1024 / 1024).toFixed(1)}MB file`,
      );
      ...
      console.log(
        `[Dropbox Browser] Using simple upload for ${(file.size / 1024 / 1024).toFixed(1)}MB file`,
      );
      ...
      if (!uploadResponse.ok) {
        ...
        return {
          success: false,
          error: `Error de Dropbox: ${uploadResponse.status}`,
        };
      }

      const uploadResult = await uploadResponse.json();
      console.log(
        "[Dropbox Browser] Simple upload success:",
    ...
    if (!sharedUrl) {
      console.warn(
        "[Dropbox Browser] File uploaded but shared link could not be created",
      );
      return {
        success: false,
        url: "",
