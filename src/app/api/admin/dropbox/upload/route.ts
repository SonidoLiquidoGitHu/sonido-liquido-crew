 ...
 *
 * NOTE: This route has been accidentally removed TWICE by auto-generated
 * UUID-named commits (143e618 and 6a02d79). Without it, every component
 * that posts to /api/admin/dropbox/upload (DropboxUploadButton,
 * DropboxUploader, BulkDropboxUploader, AudioSnippetUploader,
 * BulkAudioUploader, gallery page, EVENTS cover upload) hits a 404 →
 * the response body is Next.js' HTML 404 page → the client sees a
 * non-JSON response and shows the misleading
 * "Error de conexión con Dropbox. Reconecta tu cuenta" even though the
 * Dropbox account is perfectly healthy (visible on /admin/sync as
 * "Conectado").
 *
 * If you see this file disappear again, restore it from git history:
 *   git show 936236d:src/app/api/admin/dropbox/upload/route.ts
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[Dropbox Upload] File uploaded to Dropbox, id: ${uploadResult.id}`,
    );
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "El archivo se subió pero no se pudo crear el enlace público. Intenta usar la URL directa.",
        },
        { status: 500 },
      );
    }
