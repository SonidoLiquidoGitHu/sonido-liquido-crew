  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (releasesData.success) {
        const analytics = releasesData.data.map(
          (r: {
            id: string;
            title: string;
            artistName: string;
            ...
            conversionRate:
              (r.viewCount ?? 0) > 0
                ? ((r.presaveCount || 0) / (r.viewCount ?? 1)) * 100
                : 0,
        ...
        setReleaseAnalytics(
          analytics.sort(
            (a: ReleaseAnalytics, b: ReleaseAnalytics) =>
              b.presaveCount - a.presaveCount,
          ),
        );
      }
    } catch (error) {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-lg font-oswald">
                    {platformLabels[stats?.topPlatform || ""] || "N/A"}
                  </p>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div
                            ...
                            style={{
                              backgroundColor:
                                platformColors[platform.platform] || "#666",
                            }}
                          />
                ...
                <div className="space-y-4">
                  {sourceData.map((source, index) => {
                    ...
                    return (
                      <div
                        key={source.source}
                        className="flex items-center gap-4 p-3 bg-slc-dark rounded-lg"
                      >
                        <div
                ...
                <div className="flex items-end justify-between h-full gap-1">
                  {timelineData.slice(-30).map((day, index) => (
                    ...
                    >
                      ...
                      {(index === 0 ||
                        index ===
                          Math.floor(timelineData.slice(-30).length / 2) ||
                        index === timelineData.slice(-30).length - 1) && (
              ...
              <div className="divide-y divide-slc-border">
                {releaseAnalytics.slice(0, 10).map((release, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-oswald text-lg text-slc-muted">
                        {release.viewCount}
                      </p>
                    ...
                    <div className="text-right">
                      <p
                        className={cn(
                          "font-oswald text-lg",
                          release.conversionRate > 10
                            ? "text-green-500"
                            : release.conversionRate > 5
                              ? "text-yellow-500"
                              : "text-slc-muted",
                        )}
                      >
