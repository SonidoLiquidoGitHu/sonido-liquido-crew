...
function NewReleaseForm() {
  ...
  useEffect(() => {
    ...
    if (spotifyParam) {
      ...
      if (match) {
        setIsFetching(true);
        fetch(
          `https://open.spotify.com/oembed?url=https://open.spotify.com/album/${match[1]}`,
        )
          .then((res) => res.json())
          .then((data) => {
            setFormData((prev) => ({
            ...
            setMessage({
              type: "success",
              text: "Información obtenida de Spotify",
            });
          })
          .catch(() => {
            setMessage({
              type: "error",
              text: "Error al obtener información de Spotify",
            });
          })
          .finally(() => {
            setIsFetching(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.releaseDate) {
      setMessage({
        type: "error",
        text: "La fecha de lanzamiento es requerida",
      });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          type: "success",
          text: "Lanzamiento creado exitosamente",
        });
        setTimeout(() => {
        ...
        setMessage({
          type: "error",
          text: data.error || "Error al crear lanzamiento",
        });
      }
    } catch (error) {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slc-dark border border-slc-border rounded-xl p-4 sm:p-6">
              ...
              <p className="text-slc-muted text-sm mb-4">
                Pega la URL del álbum de Spotify para importar automáticamente
                el título y portada.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slc-dark border border-slc-border rounded-xl p-4 sm:p-6">
              ...
              <p className="text-xs text-slc-muted mt-2">
                Se importa automáticamente desde Spotify, o sube tu propia
                imagen
              </p>
            </div>
      ...
      fallback={
        <div className="p-6 lg:p-8">
          <div className="animate-pulse">
            <div className="h-8 bg-slc-dark rounded w-1/3 mb-4" />
            <div className="h-4 bg-slc-dark rounded w-1/2" />
          </div>
        </div>
      }
    >
