...
export default function NewBeatForm({ artists }: NewBeatFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.fullAudioUrl) {
      showMessage("error", "Sube el beat completo para continuar");
      return;
    }

    setIsLoading(true);
    try {
      console.log("[Beat Form] Submitting beat:", formData.title);
      ...
      console.log("[Beat Form] Response status:", response.status);

      const data = await response.json();

      if (data.success) {
        showMessage("success", "Beat creado exitosamente");
        setTimeout(() => {
