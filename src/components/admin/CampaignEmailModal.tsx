...
function generateDefaultBody(
  campaign: CampaignEmailModalProps["campaign"],
): string {
...
export function CampaignEmailModal({
  ...
  const [mailchimpCampaignUrl, setMailchimpCampaignUrl] = useState<
    string | null
  >(null);
  const [mailchimpCampaignId, setMailchimpCampaignId] = useState<string | null>(
    null,
  );
  ...
  const checkConfig = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data?.configured) {
        ...
        setConfigError(
          data.error ||
            "Mailchimp no está configurado. Ve a Email Studio para configurar tus credenciales.",
        );
      }
    } catch (err) {
      setIsConfigured(false);
    ...
    try {
      ...
      if (mailchimpCampaignId && isDraft) {
        const res = await fetch(
          `/api/admin/mailchimp/campaigns/${mailchimpCampaignId}`,
          {
            method: "POST",
          ...
          <div className="flex items-center justify-between p-6 border-b border-slc-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slc-muted">Vía Mailchimp</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {modalState === "success" && (
            <div className="p-6 space-y-4">
              <div className="flex flex-col items-center py-8 text-center">
                ...
                <h3 className="font-oswald text-2xl uppercase mb-2">¡Éxito!</h3>
                <p className="text-slc-muted max-w-md mb-6">{successMessage}</p>
              ...
              <div className="flex flex-col lg:flex-row">
                ...
                <div className="flex-1 p-6 space-y-5 border-b lg:border-b-0 lg:border-r border-slc-border overflow-y-auto max-h-[70vh]">
                  ...
                  <div className="p-4 bg-slc-card rounded-lg border border-slc-border flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slc-muted">
                        {campaign.campaignType.toUpperCase()} · /c/
                        {campaign.slug}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-slc-muted mt-1">
                      {previewText.length}/100 caracteres · Se muestra al lado
                      del asunto
                    </p>
                  </div>
                    ...
                    >
                      <h4
                        ...
                        style={{
                          color: primaryColor,
                          fontFamily:
                            fontMap[s.titleFont] || "'Oswald', sans-serif",
                        }}
                      >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          fontFamily:
                            fontMap[s.bodyFont] || "'Inter', sans-serif",
                        }}
                      >
                        {body
                          .replace(/\*\*(.*?)\*\*/g, "$1")
                            ...
                            style={{
                              ...
                              color:
                                s.buttonStyle === "outline"
                                  ...
                                  : "#ffffff",
                              borderRadius:
                                buttonRoundedMap[s.buttonRounded] || "8px",
                            }}
                          >
                            {ctaText}
                          </span>
                      ...
                      ? `Se programará para ${new Date(
                          ...
                          selectedTags.length > 0
                            ? `~${availableTags
                                ...
                                .toLocaleString()} contactos`
                            : `${audienceMemberCount.toLocaleString()} suscriptores`
                        }`
                      : selectedTags.length > 0
                        ? `Se enviará a ~${availableTags
                            .filter((t) => selectedTags.includes(t.name))
                            .reduce((sum, t) => sum + t.count, 0)
                            .toLocaleString()} contactos (${selectedTags.length} tag${selectedTags.length > 1 ? "s" : ""})`
