...
async function generateStoryCard(
  ...
  // --- Border around cover ---
  ctx.strokeStyle = event.isFeatured ? "rgba(234, 179, 8, 0.6)" : "rgba(168, 85, 247, 0.4)";
  ...
  ctx.fillText(
    event.ticketUrl ? "\uD83C\uDFAB COMPRAR BOLETOS" : "\uD83D\uDCCB VER DETALLES",
    W / 2,
    ctaY + 45
  );
...
const GENERATORS: Record<FormatTab, (canvas: HTMLCanvasElement, event: EventData) => Promise<void>> = {
...
export function EventStoryCard({
  ...
  const downloadCard = useCallback(async () => {
    ...
    a.href = url;
    const safeName = event.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 40);
  ...
  const encodedTitle = encodeURIComponent(`No te pierdas "${event.title}" en Sonido Líquido Crew`);
