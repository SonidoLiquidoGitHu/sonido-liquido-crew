...
// ===========================================
// Items queued for auto-posting to FB/IG/TikTok.
...
export const socialPostQueue = sqliteTable("social_post_queue", {
  ...
  // Reference to the source entity (gallery_photo_id, release_id, artist_id, or curated_track_id)
  ...
  platforms: text("platforms").notNull().default('["facebook","instagram","tiktok"]'), // JSON array
...
// ===========================================
// Immutable log of every post made to FB/IG/TikTok.
// One row per platform per queue item (a single queue item can produce 2-3 log rows).
...
export const socialPostsLog = sqliteTable("social_posts_log", {
  ...
  platformPostId: text("platform_post_id"), // FB post ID, IG media ID, or TikTok video ID
