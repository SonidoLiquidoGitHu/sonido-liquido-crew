...
async function runAutoMigration(client: Client): Promise<void> {
  ...
  try {
    ...
    const criticalTables = [
      // Social Auto-Posting tables (v2 — with curated_track + tiktok support)
      // Note: CHECK constraints are omitted for flexibility. Drizzle validates at the app level.
    ...
    // ===========================================
    // V2 Migration: Migrate social tables to support curated_track + tiktok
    // If the existing tables have CHECK constraints that don't include the new values,
    // we need to recreate them. This is done by:
    // 1. Creating new tables without CHECK constraints
...
async function migrateSocialTablesV2(client: Client): Promise<void> {
  try {
    ...
    await client.execute(`DROP TABLE social_posts_log`);
    await client.execute(`ALTER TABLE social_post_queue_v2 RENAME TO social_post_queue`);
    await client.execute(`ALTER TABLE social_posts_log_v2 RENAME TO social_posts_log`);
